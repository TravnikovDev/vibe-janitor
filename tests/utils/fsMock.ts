...
export class FsMock {
  ...
  setup(fileStructure: Record<string, string>): void {
    ...
    const mockWriteFile = jest.fn().mockImplementation((path: PathLike, data: any, options: any, callback?: any) => {
      if (!callback && typeof options === 'function') {
        callback = options;
        options = 'utf8';
      }
    ...
    const mockReaddir = jest.fn().mockImplementation((path: PathLike, options: any, callback?: any) => {
      if (!callback && typeof options === 'function') {
        callback = options;
        options = {};
      }
